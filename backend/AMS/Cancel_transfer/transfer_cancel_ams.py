...
def get_transfer_list_ams(request):
    ...
    try:
        with pyexasol.connect(**pyexasol_connection_string) as C:
            ...
            WHERE IS_ACTIVE=1
            AND ENTRIES_BATCH_TYPE = 'Закрытие периода'
            ;
            '''
            ...
            #     "ENTRIES_BATCH_CONTENT": row["entries_batch_content"],
            #     }
            #     for row in result
            # ]
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def xlsx_deactivate_ams(request):
    ...
    try:
        with pyexasol.connect(**pyexasol_connection_string) as C:
            ...
            query = f'''EXECUTE SCRIPT "AMS_GL"."AMS_DGL_JRNL_DEACTIVATE_CLOSE_PERIOD"('{source_file_name}', '{user_name}', '{load_timestamp}')'''

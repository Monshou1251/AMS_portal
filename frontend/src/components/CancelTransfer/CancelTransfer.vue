<template>
  <div class="file-uploader">
    <h3 class="title">Деактивация проводок из выбранного источника</h3>
    <div
      class="content"
      style="height: 413px; overflow-y: auto; border-radius: 5px"
    >
      <ag-grid-vue
        ...
        :locale-text="{ noRowsToShow: 'Нет данных для отображения' }"
      >
      </ag-grid-vue>
    </div>
    <div
...
export default {
  ...
  setup() {
    ...
    const columnDefs = ref([
      ...
      {
        headerName: "Лист файла-источника",
        field: "SHEET_NAME",
        flex: 1,
        sortable: true,
        filter: true,
      ...
      {
        headerName: "Содержание пакета проводок",
        field: "ENTRIES_BATCH_CONTENT",
        flex: 1,
        sortable: true,
        filter: true,
      },
    ...
    const getData = async () => {
      try {
        ...
        const retreiveTransferList = await axios.get(
          "/get_transfer_list",
          getData
        );
    ...
    const sendDataToBackend = async () => {
      if (dataToSend.value) {
        try {
          addLogMessage(
            "Выполняется деактивация загруженных из выбранного файла проводок."
          );
          ...
          if (response.status === 200) {
            ...
            addLogMessage(
              "Возникла ошибка про попытке деактивации проводки",
              "error"
            );
          }
          getData();
